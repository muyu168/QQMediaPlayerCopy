...
void CTitleBar::InitData()
{
	...
	m_pFileNameLabel->hide();  // 隐藏文件名标签，初始默认隐藏
...
void CTitleBar::openFile()
{
	...
	QString lastPath = settings.value("openfile_path").toString();  // 从注册表中获取路径
	...
	settings.setValue("openfile_path", tmppath);   // 将当前打开的路径写入注册表
...
void CTitleBar::openFloder()
{
	...
	settings.setValue("openfile_path", path);  // 将当前打开的路径写入注册表
