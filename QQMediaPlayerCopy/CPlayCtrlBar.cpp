...
{
	m_pTimeSlider->blockSignals(true);   // 阻塞信号
	m_pTimeSlider->setValue(value);
	m_pTimeSlider->blockSignals(false);  // 恢复信号
}
...
void CPlayCtrlBar::InitData()
{
	...
	connect(m_pTimeSlider,  &CTimeSlider::sig_SliderMove, this, &CPlayCtrlBar::onSliderMoved);
	...
	pMenu->installEventFilter(this);  //添加事件过滤器
	...
	connect(m_pPlayButtonGroupWidget, &CPlayButtonGroupWidget::sig_VolumeChanged, this, &CPlayCtrlBar::sig_VolumeChanged);
	connect(m_pPlayButtonGroupWidget, &CPlayButtonGroupWidget::sig_play, this, &CPlayCtrlBar::sig_play);
	connect(m_pPlayButtonGroupWidget, &CPlayButtonGroupWidget::sig_pause, this, &CPlayCtrlBar::sig_pause);
...
void CPlayCtrlBar::mousePressEvent(QMouseEvent* event)
{
	// 检查是否在边缘区域（用于窗口缩放）
	...
	// 如果在左右边缘或底部边缘，让事件传递给父窗口处理缩放
	if (x < edgeMargin || x > width() - edgeMargin || y > height() - edgeMargin)
	{
		event->ignore();  // 忽略事件，让父窗口处理
		return;
	}
...
void CPlayCtrlBar::setPlayingState(bool isPlaying)
{
	if (m_pPlayButtonGroupWidget) {
		m_pPlayButtonGroupWidget->setPlayingState(isPlaying);
	}
}
